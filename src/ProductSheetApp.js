...
const getEmbedUrl = (url) => {
  ...
  // TikTok Clean View Fix (v2 player hides side clutter)
...
export default function ProductSheetApp() {
  ...
  const isCustomerView = currentView === 'customer' || !currentView;
      ...
      <div className="pt-12 pb-6 px-6 bg-black sticky top-0 z-[100] border-b border-white/5">
        <div className="max-w-6xl mx-auto flex flex-col items-center">
          ...
          <div className="flex gap-2 mt-6 overflow-x-auto no-scrollbar w-full justify-center">
            {categories.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-5 py-2 rounded-full text-[9px] font-black uppercase transition-all ${activeCategory === cat ? 'bg-red-800 text-white shadow-lg shadow-red-900/20' : 'bg-neutral-900 text-neutral-500 hover:text-white'}`}>{cat}</button>
            ))}
          </div>
          {isAdmin && isAdminView && (
          ...
          <div className="bg-neutral-900/50 rounded-[2.5rem] p-8 mb-20 max-w-xl mx-auto border border-white/5 shadow-2xl">
            ...
            <form onSubmit={handleAddProduct} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <select value={formData.badge} onChange={(e) => setFormData({...formData, badge: e.target.value})} className="w-full p-4 rounded-xl bg-black border border-white/5 text-neutral-400 outline-none">
                  <option value="">No Badge</option><option value="New Arrival">New Arrival</option><option value="Best Seller">Best Seller</option>
                </select>
              </div>
              <button type="submit" className="w-full bg-red-800 text-white font-black py-4 rounded-xl uppercase tracking-widest hover:bg-red-700 transition-colors shadow-lg shadow-red-900/20">{status || (editingId ? "Update Data" : "Post Product")}</button>
              {editingId && <button type="button" onClick={() => window.location.reload()} className="w-full text-neutral-600 uppercase text-[9px] font-bold tracking-widest mt-2">Cancel Edit</button>}
            </form>
          </div>
        )}
          ...
          {(activeCategory === "All" ? products : products.filter(p => p.category === activeCategory)).map(product => (
            <div key={product.id} 
              ...
              <div className="aspect-square bg-black rounded-2xl overflow-hidden mb-3 relative">
                ...
                {product.badge && <div className="absolute top-3 left-3 bg-red-700 text-white text-[7px] font-black px-2 py-1 rounded-full uppercase tracking-tighter shadow-md">{product.badge}</div>}
              </div>
              ...
              {isAdmin && isAdminView && (
                <div className="absolute top-4 right-4 flex gap-2">
                   <button onClick={(e) => { e.stopPropagation(); handleEdit(product); }} className="bg-black/90 p-2 rounded-full text-neutral-400 border border-white/5 hover:text-white transition-colors shadow-lg"><Edit3 size={14} /></button>
                   <button onClick={(e) => { e.stopPropagation(); if(window.confirm("Permanent Delete?")) supabase.from('products').delete().eq('id', product.id).then(() => window.location.reload()); }} className="bg-black/90 p-2 rounded-full text-red-900 border border-white/5 hover:text-red-600 transition-colors shadow-lg"><Trash2 size={14} /></button>
                </div>
              )}
            </div>
          ))}
  ...
  {/* FOOTER - Premium Interaction Effect */}
          ...
          <div className="bg-neutral-950 border border-white/5 rounded-[3rem] max-w-sm w-full overflow-hidden relative shadow-[0_0_100px_rgba(185,28,28,0.1)]">
            ...
            <div className="h-[520px] w-full bg-black relative flex items-center justify-center overflow-hidden">
                {selectedProduct.video_url && isPlaying ? (
                  <div className="w-full h-full scale-[1.06]"> {/* Zoomed to crop out TikTok watermarks/buttons */}
                    <iframe 
            ...
            <div className="p-8 text-center bg-neutral-950/80 backdrop-blur-md">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => sendOrder('whatsapp', selectedProduct)} className="bg-[#25D366] text-white py-4 rounded-[1.2rem] font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:brightness-110 shadow-lg shadow-green-950/20 active:scale-95 transition-all">WhatsApp</button>
                <button onClick={() => sendOrder('messenger', selectedProduct)} className="bg-[#0084FF] text-white py-4 rounded-[1.2rem] font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:brightness-110 shadow-lg shadow-blue-950/20 active:scale-95 transition-all">Messenger</button>
              </div>
            </div>
          </div>
        </div>
